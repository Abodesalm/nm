...
- **Add/delete core logic lives in `src/lib/storageActions.ts`** (`addStorageActionToItem`, `deleteStorageActionFromItem`) — the per-item route's `DELETE` and the global route's `POST`/`DELETE` all call the same functions, so behavior (quantity recalculation, point-equipment sync, History, Invoice/Loan/Treasury cascade) is guaranteed identical regardless of entry point. Both accept an optional `enforceDirection` used only by the locked دخل/خرج pages (see below).
