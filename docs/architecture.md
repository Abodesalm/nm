...
- **Add/delete core logic lives in `src/lib/storageActions.ts`** (`addStorageActionToItem`, `deleteStorageActionFromItem`) — both the per-item route (`api/storage/[id]/actions`) and the global cross-item route (`api/storage/actions`) call the same functions, so behavior (quantity recalculation, point-equipment sync, History, Invoice/Loan/Treasury cascade) is guaranteed identical regardless of entry point. Both accept an optional `enforceDirection` used only by the locked دخل/خرج pages (see below).
