...
- **A few keys are RESTRICTIONS, not grants** — turning them ON removes a capability instead of adding one, so they can never go through `permissionGuard`'s grant-shaped fallback and always carry their own dedicated guard. Currently one: storage `action_delete_same_day_only` (§ Storage). Super admins bypass restrictions; `full` section access does not (level says what you may reach, a restriction says how you may use it).
...
- **Same-day-only delete limit (opt-in restriction):** the `action_delete_same_day_only` permission key locks a user out of deleting any action once the day it was recorded is over — enforced by `requireDeletableToday()` on both delete routes (right after the direction gate), mirrored in the UI as a disabled lock icon in place of the trash button. Off by default; super admins bypass, `full` access does not. **"Which day" is read from the action's `_id`** (ObjectIds embed their creation moment — no migration needed), never from `action.date`, which is user-entered and backdatable in both directions; `objectIdDate()`/`isSameLocalDay()` in `utils.ts` are shared so client and server always agree. Item deletion (`item_delete`) still cascades old actions regardless — the limit is scoped to action deletion.
